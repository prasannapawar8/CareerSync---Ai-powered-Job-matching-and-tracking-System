...
export default async function JobsPage() {
  ...
  const resume = await prisma.resume.findFirst({ where: { userId: session.user.id }, orderBy: { uploadedAt: 'desc' }, select: { rawText: true } });
  ...
  const skills = extractResumeSkills(resume.rawText);
  let jobs: JobMatch[] = [];
  let searchError = '';
  ...
  return <JobsPageShell><div className="mb-7 flex flex-wrap gap-2">{skills.map((skill) => <span key={skill} className="rounded-full border border-border bg-surface px-3 py-1 text-xs text-muted">{skill}</span>)}</div>{jobs.length ? <div className="space-y-3">{jobs.map((job) => <article key={job.id} className="rounded-xl border border-border bg-surface p-5 shadow-sm"><div className="flex flex-col justify-between gap-4 sm:flex-row"><div><div className="flex items-center gap-3"><h2 className="font-semibold text-foreground">{job.title}</h2><span className="rounded-full bg-surface-muted px-2.5 py-1 text-xs font-medium text-foreground">{job.matchScore}% match</span></div><p className="mt-1 text-sm text-muted">{job.company} · {job.location}</p><p className="mt-3 line-clamp-2 text-sm leading-6 text-muted">{job.description}</p><p className="mt-3 text-xs text-subtle">Matched: {job.matchedSkills.join(', ')}</p></div><a href={job.applyUrl} target="_blank" rel="noreferrer" className="h-fit shrink-0 rounded-lg bg-accent px-4 py-2.5 text-center text-sm font-medium text-accent-foreground transition-opacity hover:opacity-85">View job</a></div></article>)}</div> : <EmptyState title="No strong matches yet" description="Try uploading a resume with more role-specific skills, then return here." />}</JobsPageShell>;
}
